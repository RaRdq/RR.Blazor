...
class ModuleManager {
    constructor() {
        ...
        this.moduleUrls = {
            ...
            focusTrap: '/_content/RR.Blazor/js/focus-trap.js'
        };
...
window.RRBlazor = {
    ...
    Portal: {
        ...
        async destroy(portalId) {
            const portal = await moduleManager.getModule('portal');
            const portalManager = portal.portalManager || window.RRPortalManager;
            const result = await portalManager.destroy(portalId);
            // C# expects a boolean - convert any truthy result to boolean
            return !!result;
        },
        ...
        async hasOpenPortalsInContainer(containerSelector) {
            const portal = await moduleManager.getModule('portal');
            const portalManager = portal.portalManager || window.RRPortalManager;
            return portalManager.hasOpenPortalsInContainer(containerSelector);
        },
        
        async cleanupOrphanedElements() {
            const portal = await moduleManager.getModule('portal');
            const portalManager = portal.portalManager || window.RRPortalManager;
            return portalManager.cleanupOrphanedElements();
        }
    },
    ...
    // FocusTrap API
    FocusTrap: {
        async create(modalElement, trapId) {
            try {
                const focusTrap = await moduleManager.getModule('focusTrap');
                if (!focusTrap) throw new Error('FocusTrap module failed to load');
                return focusTrap.createFocusTrap(modalElement, trapId);
            } catch (error) {
                debugLogger.error('FocusTrap creation failed:', error);
                return false;
            }
        },
        
        async destroy(trapId) {
            try {
                const focusTrap = await moduleManager.getModule('focusTrap');
                if (!focusTrap) return false;
                return focusTrap.destroyFocusTrap(trapId);
            } catch (error) {
                debugLogger.error('FocusTrap destruction failed:', error);
                return false;
            }
        }
    },
    ...
    // Focus Trap API for accessible modal navigation
    FocusTrap: {
        async create(modalElement, trapId) {
            const focusTrap = await moduleManager.getModule('focusTrap');
            return focusTrap.createFocusTrap(modalElement, trapId);
        ...
        async isActive(trapId) {
            const focusTrap = await moduleManager.getModule('focusTrap');
            return focusTrap.isFocusTrapActive(trapId);
        },
        
        async getActiveCount() {
            const focusTrap = await moduleManager.getModule('focusTrap');
            return focusTrap.getActiveFocusTrapCount();
        }
    },
